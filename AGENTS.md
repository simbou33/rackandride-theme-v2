...
# Rack And Ride — Project Rules

> Everything above is the generic Shopify/Liquid best-practices baseline. **Everything below is project-specific and OVERRIDES the generic guidance wherever they conflict.** Refonte du thème sur base **Dawn fraîche**.
...
- Boutique (admin/dev) : `rack-and-ride.myshopify.com`
- Domaine public : `shop.rackandride.com`

## 2. Langues — 4 marchés (règle absolue)

Marchés : **EN (défaut), FR, DE, ES**.

🚨 **RÈGLE ABSOLUE — ZÉRO texte en dur.** Toute chaîne visible par le client passe par une clé de traduction (`| t`) présente dans **les 4** fichiers :
`locales/en.default.json` + `locales/fr.json` + `locales/de.json` + `locales/es.json`.

- Les `.schema.json` = libellés **éditeur** (theme editor), **séparés** des locales storefront. Ne jamais mélanger.
- Une clé ajoutée doit exister dans les 4 storefront locales (au moins EN rempli ; les autres traduits, jamais laissés vides en prod).
- Ne **jamais** casser le **hreflang automatique** de Shopify.
- `page_title` / `page_description` par langue — jamais de title/meta en dur.

## 3. Palette « Chêne & Océan » — premium lumineuse

Définir comme tokens CSS avec préfixe **`--rar-`** (voir §6).

| Rôle | Hex | Usage |
|------|-----|-------|
| Fond principal | `#FFFFFF` | base, beaucoup de blanc |
| Surface secondaire | `#F8F5F0` | sections alternées, cartes |
| Accent chêne | `#B07C4C` | **UNIQUEMENT** boutons / liens / underlines / icônes — **JAMAIS en fond de bloc** |
| Encre (texte/titres) | `#1C2830` | texte, titres, **et prix** |
| Sauge (éco/badges) | `#7A8B6E` | badges éco-responsables |
| Terracotta (promo) | `#C16E4F` | badges promo **uniquement** |

**Direction artistique :** beaucoup de blanc, **espace négatif généreux**, accents chêne **rares et précieux**.

## 4. Typographie (self-hosted, RGPD)

- **Titres : Fraunces**
- **Corps : Work Sans**
- Fichiers `.woff2` dans `/assets`, sous-ensembles **latin + latin-ext**. Aucune dépendance Google Fonts externe (conformité RGPD).
- `@font-face` avec `font-display: swap` ; précharger les poids critiques.

## 5. Règles UI immuables

- **Prix** affiché en **encre `#1C2830`** (jamais en accent chêne ni en promo, sauf badge promo dédié).
- **Header** : comportement **on-scroll-up** (réapparaît au scroll vers le haut) — **PAS** always-sticky.
- **Panier** : **cart drawer** (tiroir latéral) — **PAS** de page panier.
- **Largeur max de contenu : 1400px.**
- **Logo** : **SVG inline** depuis `/assets` (fichier à fournir) — **jamais** le nom de marque en texte.
- **Arrondis** : boutons **6px**, cartes **~12px**.

## 6. Tokens CSS

Tous les tokens préfixés `--rar-` (couleurs, rayons, largeur max, typo). Déclarés une fois globalement. Settings d'une seule propriété CSS → variable CSS inline ; settings multi-propriétés → classes CSS (cf. baseline ci-dessus).

## 7. SEO (toutes phases)

**JSON-LD requis :** `Product` + `AggregateRating`/`Review`, `BreadcrumbList`, `Organization`, `FAQPage`, `Article`.

**Collections :**
- `H1` = titre de la collection (**1 seul H1 par page**).
- Texte SEO d'intro court avec « **lire la suite** ».
- Bloc rich text SEO **pilotable** en bas de page.
- **FAQ pilotable** avec schema `FAQPage`.
- **Breadcrumbs 2 niveaux** : `Accueil > Collection` (jamais de doublon de niveau).

**Règles transverses :**
- Jamais de title/meta en dur — `page_title`/`page_description` par langue.
- Ne pas casser le **hreflang** auto.
- **Handles stables.**

**Perf / CWV :** WebP responsive (`srcset`), lazy-load, `width`/`height` explicites sur les images, **JS en `defer`**.

## 8. Handles RÉELS des collections (vérifiés — à utiliser partout)

| Catégorie | Handle |
|-----------|--------|
| Surf | `surfboard-wall-mount` ⚠️ **singulier** |
| Skate | `skateboard-wall-mounts` |
| Snowboard | `snowboard-wall-mounts` |
| Skis | `skis-wall-mounts` |
| Vélo | `bike-racks` |
| Wake & Kite | `kitesurf-racks` |
| Déco | `deco` |

➡️ **Pour la navigation, privilégier les menus Shopify (`linklists`)** configurables en admin plutôt que des handles codés en dur. N'utiliser les handles ci-dessus que là où une référence directe est nécessaire.

## 9. Workflow local

- **Dev :** `shopify theme dev --store rack-and-ride.myshopify.com` → aperçu `http://127.0.0.1:9292` (hot-reload).
- **Publication :** `shopify theme push --unpublished`.
- **Commits git réguliers.**

## 10. Plan de construction (phases)

- **Phase 0 — Fondations :** palette, polices self-hosted, arrondis, cart drawer, tokens CSS (préfixe `--rar-`).
- **Phase 1 — Chrome global :** header (dropdown catégories), footer, barre d'annonce multilingue, cart drawer + barre livraison offerte (**seuil 100 €**), logo SVG.
- **Phase 2 — Page produit :** ✅ fait. Galerie média (image + vidéo). **Ordre (révisé CRO — l'ATC doit être visible quasi sans scroll)** : titre/prix/variant → **short description courte** (`custom.short_description`) → qté (stepper) → **ATC/Buy now** + paiement + micro-réassurance « paiement sécurisé » → bandeau réassurance → **Points forts** (`custom.key_features`, liste complète placée APRÈS l'ATC) → accordéons (**Matériaux / Livraison & Retours / Dimensions / Entretien**, override metafield possible : Dimensions → `custom.dimensions`) → description longue (pleine largeur, sous le bloc) → section **story-rows** (storytelling SEO, réutilisable). **Sticky ATC** ; JSON-LD Product+Offer (pas d'AggregateRating tant qu'aucune app d'avis). _Note : l'ancien ordre « USP avant ATC » a été abandonné car un `key_features` long enterrait le CTA._
- **Phase 3 — Accueil :** hero, bandeau réassurance, tuiles catégories, featured products, histoire de marque, avis, newsletter. **ZÉRO placeholder Dawn.**
- **Phase 4 — Collection :** grille, filtres aux noms SEO, bloc SEO (intro + lire la suite + rich text + FAQ schema), breadcrumbs.
- **Phase 5 — Nettoyage + SEO :** structured data complet, hreflang, perf/CWV, theme-check, accessibilité.
